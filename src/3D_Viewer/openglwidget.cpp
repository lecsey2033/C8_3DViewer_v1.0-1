...
void OpenGLWidget::paintGL() {
  ...
  glLoadIdentity();
  if (!projection) {
    glOrtho(-objdata.max, objdata.max, -objdata.max, objdata.max, objdata.max,
            5 * objdata.max);
  } else {
    glFrustum(-objdata.max, objdata.max, -objdata.max, objdata.max, objdata.max,
              5 * objdata.max);
  }
...
void OpenGLWidget::drawCube() {
  ...
  glDrawElements(GL_LINES, objdata.count_of_vertexes_in_facets, GL_UNSIGNED_INT,
                 objdata.indexes_array);
