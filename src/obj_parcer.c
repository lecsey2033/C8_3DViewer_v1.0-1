...
void parcig_of_facets(char *str, obj_data *data, int m) {
    ...
    data->polygons[m].polygon_i = (unsigned int*)calloc(data->polygons[m].num_vertexes, sizeof(unsigned int));
...
void writing_indexes_array(obj_data *data) {
    int n = 0;
    data->indexes_array = (unsigned int*)calloc(data->count_of_vertexes_in_facets, sizeof(unsigned int));
    for (unsigned int i = 0; i < data->amount_of_facets; i++) {
        ...
        data->indexes_array[n] = data->polygons[i].polygon_i[data->polygons[i].num_vertexes - 1];
        n++;
...
// сдвиг массива вершин в положение, когда объект будет находится по центру системы координат в OpenGL
// нахождение модуля максимального значения среди min и max всех трёх осей
